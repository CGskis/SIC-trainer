...
export function createSessionRecord({ session, scenario, applicationVersion, startedAt, endedAt }) {
  return { schemaVersion: SCHEMA_VERSION, sessionId: crypto.randomUUID(), applicationVersion, scenarioId: scenario.id, scenarioVersion: scenario.version, startedAt, endedAt, inputEvents: session.events, finalState: session.state, finalScore: null, durationMs: new Date(endedAt).getTime() - new Date(startedAt).getTime(), errors: session.events.filter((event) => event.type === "INVALID_ACTION") };
}
