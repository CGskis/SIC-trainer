...
function show(id){document.querySelectorAll(".screen").forEach(item=>item.classList.toggle("active",item.id===id));document.querySelectorAll("[data-screen]").forEach(item=>item.classList.toggle("selected",item.dataset.screen===id));if(id==="history")renderHistory();if(id==="debrief")renderDebrief()}
